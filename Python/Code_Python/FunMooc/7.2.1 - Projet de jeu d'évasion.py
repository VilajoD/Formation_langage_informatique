...
Petit jeu où il faut parcourir un château, repondre correctement aux questions des portes,
    récupérer tous les trésors et sortir à l'autre bout.
...
# ATTENTION :
# Changer le nom du repertoire pour acceder aux fichiers du plan de chateau et des dico objets et portes.
import os
...
# Fichier de configs des variables globale à mettre dans le même dossier que ce code
from CONFIGS import *
...
def coordonnees(case, pas):
    ...
    la dimension "pas" reçu en entrée. Et renvoie cette position.
...
def tracer_joueur(position_joueur, dimension):
    ...
    # Pour trouver le centre : cordoonées en bas a gauche de la case + la taille de la case/2
    # Pour abscisse le + 1 est juste estétique afin que le pion soit bien au centre de la case
    # Pour ordonnée le -dimension est du a la maniere dont son crées les cases à la base afin que le rebord en haut a
        # gauche du plan soit toujours a la même place peut importe la taille des cases.
    coordonnees_centre_rond = (coordonnees_case[0] + (dimension/2)-1,(coordonnees_case[1] + (dimension/2))-dimension)
...
def afficher_plan(matrice):
    ...
    turtle.shape("blank")
    dimension = calculer_pas(matrice) #Calcul de la dimension des cases du plan à afficher
    turtle.tracer(0)  #Augmentation de la vitesse d'affichage
    ...
    for x,ligne in enumerate(matrice): #Pour chaque case de la matrice, calcul des coordonnées,
        ...
        for y,case in enumerate(ligne):
            if case != 0 : # Pour gagner du temps d'affichage, les cases blanche ne sont pas affichéés
                           # puisque le fond et aussi blanc
...
def ramasser_objet(prochaine_position, liste_objet, inventaire):
    """
    Ajoute l'objet dans la liste de l'inventaire
...
def question_porte(matrice,position_joueur,prochaine_position,dimension, dico_porte,):
    """
    Récupere la question a poser dans le dico des portes. Pose la question au joueur et ananlyse sa réponse.
    Si la reponse est mauvaise, affiche le message de mauvause reponse
    Si la reponse est bonne, affiche que la reponse est bonne en annonce, fait disparaître la portes du plan
        et de la matrice et deplace le personnage.
    :param matrice: Matrice du plan
    :param prochaine_position: Position ou va aller le joueur (correspond au coordonnées de la porte)
    :param dimension: taille d'une case
    :param dico_porte: Liste de toutes les portes présentes dans le château
    :return: Nouvelle position ou se trouve le joueur
    """
...
def afficher_message(texte, police=('Arial', 8, 'normal')):
    """
    Fonction pour faire apparaître un message reçu en entrée dans la zone de d'annonce.
    :param texte: Message à faire apparaître
    :param police: Police du message. Pas defaut en ('Arial', 8, 'normal')
    :return:
    """
